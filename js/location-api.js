	...
	//FORMS
		// CREATE PLACE
			$(document).on('submit', '#form-create-place', function(e){
				...
				var phone = $.trim( $('#fo-phone').val() );
				var ambience = $.trim( $('#fo-ambience').val() );
				var elderly = $.trim( $('#fo-elderly').val() );
				var equipments = $.trim( $('#fo-equipments').val() );
				var medicines = $.trim( $('#fo-medicines').val() );
			...
			$(document).on('change', '#fo-place', function(){
				...
				if( Number( $handle.val() ) == 0){
					...
					$.ajax({
						...
						$('#fo-phone').val( data.place.phone );
						$('#fo-ambience').val( data.place.ambience );
						$('#fo-elderly').val( data.place.elderly );
						$('#fo-equipments').val( data.place.equipments );
						$('#fo-medicines').val( data.place.medicines );
